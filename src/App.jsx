...
function fmtDateLabel(ds, t, locale = 'en-CA') {
  ...
  if (diff === 0) return t ? t('date.today') : 'Today'
  if (diff === 1) return t ? t('date.tomorrow') : 'Tomorrow'
  if (diff === -1) return t ? t('date.yesterday') : 'Yesterday'
...
function AuthScreen() {
  ...
  async function submit() {
    ...
    if (mode === 'signup') {
      ...
      else setSuccess(t('auth.account_created'))
    } else {
    ...
    <div style={{ ...S.root, paddingBottom: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24, minHeight: '100vh' }}>
      <div style={{ width: '100%', maxWidth: 360 }}>
        <div style={{ textAlign: 'center', marginBottom: 36 }}>
          <div style={{ fontSize: 26, fontWeight: 700, color: C.purple, letterSpacing: -0.5 }}>{t('app.name')}</div>
          <div style={{ fontSize: 12, color: C.textLow, marginTop: 4 }}>{t('app.subtitle')}</div>
        </div>
        <div style={S.card}>
          ...
          <div style={S.lbl}>{t('auth.email')}</div>
          <input style={{ ...S.inp, marginBottom: 10 }} type="email" placeholder={t('auth.email_placeholder')}
            value={email} onChange={e => setEmail(e.target.value)}
            onKeyDown={e => e.key === 'Enter' && submit()} autoFocus />
          <div style={S.lbl}>{t('auth.password')}</div>
          <input style={{ ...S.inp, marginBottom: 14 }} type="password" placeholder={t('auth.password_placeholder')}
    ...
    <div style={{ ...S.root, paddingBottom: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 24, minHeight: '100vh' }}>
      <div style={{ width: '100%', maxWidth: 380 }}>
        <div style={{ textAlign: 'center', marginBottom: 28 }}>
          <div style={{ fontSize: 20, fontWeight: 700 }}>{t('book.welcome')}</div>
          <div style={{ fontSize: 12, color: C.textLow, marginTop: 4 }}>{t('book.subtitle')}</div>
        </div>
        <div style={S.card}>
          <div style={S.lbl}>{t('book.name_label')}</div>
    ...
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.85)', zIndex: 900, display: 'flex', flexDirection: 'column', justifyContent: 'flex-end' }}>
      <div style={{ background: C.surface, borderRadius: '20px 20px 0 0', padding: '20px 18px 32px', maxHeight: '92vh', overflowY: 'auto' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
          <div style={{ fontSize: 15, fontWeight: 700 }}>{defaultRecurrence && defaultRecurrence !== 'once' ? t('tx.add_bill') : t('tx.add')}</div>
          <button onClick={onClose} style={{ background: 'none', border: 'none', color: C.textMid, fontSize: 22, cursor: 'pointer' }}>×</button>
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 10 }}>
          <div>
            <div style={S.lbl}>{t('tx.type')}</div>
          ...
          <div>
            <div style={S.lbl}>{t('tx.amount')}</div>
        ...
        {form.recurrence !== 'once' && (
          <div style={{ marginBottom: 10 }}>
            <div style={S.lbl}>{t('tx.end_date')}</div>
        ...
        <button style={{ ...S.btn(form.type === 'income' ? C.green : C.orange), width: '100%', marginTop: 6 }} onClick={save} disabled={saving}>
          {saving ? t('common.saving') : form.type === 'income' ? t('tx.btn_income') : form.type === 'transfer' ? t('tx.btn_transfer') : t('tx.btn_expense')}
        </button>
      </div>
    </div>
  )
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.88)', zIndex: 1000, display: 'flex', flexDirection: 'column', justifyContent: 'flex-end' }} onClick={onClose}>
      <div style={{ background: C.surface, borderRadius: '20px 20px 0 0', padding: '20px 18px 36px', maxHeight: '80vh', overflowY: 'auto' }} onClick={e => e.stopPropagation()}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 16 }}>
          <div>
            ...
            <div style={{ fontSize: 11, color: C.textLow, marginTop: 2 }}>
              {fmtDateLabel(date, t, locale)} · {tx.account}{tx.category ? ` · ${tx.category}` : ''}
        ...
        >
          {isSkipped ? t('edit.unskip') : t('edit.skip')}
        </button>
        ...
        {!showDelete ? (
          ...
          <div style={{ background: C.redBg, border: `1px solid ${C.red}`, borderRadius: 10, padding: '12px 14px' }}>
            <div style={{ fontSize: 13, color: C.red, fontWeight: 600, marginBottom: 8 }}>{t('edit.delete_confirm', { name: tx.label })}</div>
            <div style={{ display: 'flex', gap: 8 }}>
              <button style={{ ...S.btn(C.red), flex: 1 }} onClick={deleteTx} disabled={saving}>
                {saving ? t('common.deleting') : t('edit.yes_delete')}
              </button>
              <button style={{ ...S.btn(C.surfaceHigh, true) }} onClick={() => setShowDelete(false)}>{t('common.cancel')}</button>
            </div>
          </div>
        )}
      </div>
...
function OverviewTab({ accounts, transactions, overrides, onReconcile, bookId, onGoToCycles, onGoToAccounts }) {
  ...
  return (
    <div>
      ...
      {/* Accounts */}
      <div style={S.card}>
        <div style={{ ...S.lbl, marginBottom: 8 }}>{t('overview.accounts')}</div>
        {accounts.length === 0 && (
          <div style={{ textAlign: 'center', padding: '12px 0 4px' }}>
            <div style={{ fontSize: 12, color: C.textLow, marginBottom: 10 }}>{t('overview.no_accounts')}</div>
      ...
      {(() => {
        ...
        return todayItems.length > 0 && (
          <div style={S.card}>
            <div style={{ ...S.lbl, marginBottom: 8 }}>{t('overview.today')}</div>
      ...
      {billsUntilNext.length > 0 && (
        <div style={S.card}>
          <div style={{ ...S.lbl, marginBottom: 8 }}>{t('overview.bills_til', { date: nextIncomeDate ? fmtDateLabel(nextIncomeDate, t, locale) : t('overview.next_income') })}</div>
          {billsUntilNext.map((item, i) => (
            <div key={`${item.tx.id}:${item.date}`} style={{ ...S.row, borderBottom: i < billsUntilNext.length - 1 ? `1px solid ${C.border}` : 'none' }}>
              <div>
                ...
                <div style={{ fontSize: 10, color: C.textLow }}>{fmtDateLabel(item.date, t, locale)} · {item.tx.account}</div>
              </div>
              <div style={{ fontSize: 14, fontWeight: 700, color: C.red }}>−{fmt(item.amt, locale)}</div>
            </div>
          ))}
          <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: 8, paddingTop: 8, borderTop: `1px solid ${C.border}` }}>
            <span style={{ fontSize: 12, color: C.textLow }}>{t('overview.total')} <span style={{ color: C.red, fontWeight: 700 }}>{fmt(billsTotal, locale)}</span></span>
          </div>
        </div>
      )}
    ...
    <div>
        ...
        return (
          <div style={{ ...S.card, background: C.surfaceHigh, marginBottom: 14 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8 }}>
              <div>
                <div style={{ fontSize: 10, color: C.textLow, textTransform: 'uppercase', letterSpacing: 2 }}>{t('ahead.pay_cycle')}</div>
      ...
      {byDate.length === 0 && (
        <div style={{ textAlign: 'center', padding: '48px 0', color: C.textLow, fontSize: 13 }}>
          {t('ahead.no_upcoming')}<br />
          <span style={{ fontSize: 11 }}>{t('ahead.add_hint')}</span>
        </div>
      )}
    ...
    <div>
      ...
      <div style={{ display: 'flex', gap: 14, marginTop: 12, justifyContent: 'center' }}>
        <div style={{ fontSize: 9, color: C.textLow }}>
          <span style={{ color: C.green }}>+$X</span> = {t('cal.legend')}
        </div>
      </div>
    </div>
  )
...
function AccountsTab({ accounts, bookId, onReconcile, onRefresh, initAdd, onInitAddDone }) {
  ...
  async function addAccount() {
    if (!newName.trim()) { setAddErr(t('accounts.name_required')); return }
  ...
  return (
    <div>
        ...
        return (
          <div key={a.id} style={{ ...S.card, borderLeft: `3px solid ${isNeg ? C.red : C.green}` }}>
            {editId === a.id ? (
              <div>
                <div style={{ display: 'flex', gap: 8, marginBottom: 8 }}>
                  <input style={{ ...S.inp, flex: 1 }} value={editName} onChange={e => setEditName(e.target.value)} autoFocus placeholder={t('accounts.name')} />
                </div>
                <div style={{ marginBottom: 8 }}>
                  <div style={S.lbl}>{t('accounts.classification')}</div>
                  <input style={S.inp} value={editClassification} onChange={e => setEditClassification(e.target.value)} placeholder={t('accounts.classification_placeholder')} />
                </div>
                <div style={{ display: 'flex', gap: 8 }}>
                  <button style={S.btn(C.green)} onClick={() => saveEdit(a.id)} disabled={saving}>{t('common.save')}</button>
                  <button style={S.btn(C.surfaceHigh, true)} onClick={() => setEditId(null)}>{t('common.cancel')}</button>
                </div>
              </div>
            ) : (
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 8 }}>
                ...
                <button onClick={() => { setEditId(a.id); setEditName(a.name); setEditClassification(a.classification || '') }} style={{ background: 'none', border: 'none', color: C.textLow, fontSize: 11, cursor: 'pointer' }}>{t('accounts.edit')}</button>
              </div>
            )}
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 4 }}>
              <div>
                <div style={S.lbl}>{t('accounts.balance')}</div>
                <div style={{ fontSize: 24, fontWeight: 700, color: isNeg ? C.red : C.green }}>{fmt(a.balance, locale) || '--'}</div>
              </div>
            ...
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 8, flexWrap: 'wrap', gap: 6 }}>
              {a.baseline_date
                ? <div style={{ fontSize: 10, color: C.textLow }}>{t('accounts.last_confirmed', { date: fmtMonthDay(a.baseline_date, locale) })}</div>
                : <div />
              }
              <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                ...
                >
                  {a.track_only ? t('accounts.tracking_only') : t('accounts.in_balance')}
                </button>
                <button
                ...
                >
                  {a.include_in_safe_to_spend ? t('accounts.in_sts') : t('accounts.not_in_sts')}
                </button>
              </div>
            </div>
          </div>
      ...
      {showAdd ? (
        <div style={S.card}>
          <div style={{ fontWeight: 700, fontSize: 13, marginBottom: 12 }}>{t('accounts.new_account')}</div>
          <div style={{ marginBottom: 10 }}>
            <div style={S.lbl}>{t('accounts.name')}</div>
            <input style={S.inp} placeholder={t('accounts.name_placeholder')} value={newName} onChange={e => setNewName(e.target.value)} autoFocus />
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 10 }}>
            <div>
              <div style={S.lbl}>{t('accounts.type')}</div>
            ...
            <div>
              <div style={S.lbl}>{t('accounts.current_balance')}</div>
              <input style={S.inp} type="number" inputMode="decimal" placeholder={newType === 'credit' ? t('accounts.amount_owed') : '0.00'} value={newBalance} onChange={e => setNewBalance(e.target.value)} />
              {newType === 'credit' && (
                <div style={{ fontSize: 10, color: C.orange, marginTop: 3 }}>{t('accounts.stored_negative')}</div>
              )}
            </div>
          </div>
          <div style={{ marginBottom: 10 }}>
            <div style={S.lbl}>{t('accounts.classification')}</div>
            <input style={S.inp} placeholder={t('accounts.classification_placeholder')} value={newClassification} onChange={e => setNewClassification(e.target.value)} />
          </div>
          <div style={{ marginBottom: 14 }}>
            <div style={S.lbl}>{t('accounts.include_sts')}</div>
            <button
            ...
            >
              {newIncludeSTS ? t('accounts.yes_sts') : t('accounts.no_sts')}
            </button>
          </div>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            <button style={{ ...S.btn(C.green), flex: 1 }} onClick={addAccount} disabled={saving}>
              {saving ? t('common.saving') : t('accounts.add_btn')}
            </button>
            <button style={{ ...S.btn(C.surfaceHigh, true) }} onClick={() => { setShowAdd(false); setAddErr(null); setNewClassification(''); setNewIncludeSTS(true) }}>{t('common.cancel')}</button>
          </div>
        </div>
      ) : (
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', gap: 6, marginBottom: 6, alignItems: 'center' }}>
        {[['', t('flow.all')], ['income', t('flow.in')], ['expense', t('flow.out')]].map(([v, label]) => (
        ...
        <select style={{ ...S.sel, flex: 1, minWidth: 0, fontSize: 12, padding: '5px 8px' }} value={filterAccount} onChange={e => setFilterAccount(e.target.value)}>
          <option value="">{t('flow.account')}</option>
      ...
      <div style={{ display: 'flex', gap: 6, marginBottom: 14, alignItems: 'center' }}>
        <select style={{ ...S.sel, flex: 1, minWidth: 0, fontSize: 12, padding: '5px 8px' }} value={filterCategory} onChange={e => setFilterCategory(e.target.value)}>
          <option value="">{t('flow.category')}</option>
          {catOptions.map((c, i) => <option key={i} value={c.name}>{c.name}</option>)}
        </select>
        {[['date', t('flow.date')], ['amount', t('flow.amt')]].map(([v, label]) => (
          ...
          <div key={`${inst.tx.id}:${inst.date}`}
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              ...
              <div style={{ fontSize: 10, color: C.textLow, marginTop: 1 }}>
                {fmtDateLabel(inst.date, t, locale)}{inst.tx.account ? ` · ${inst.tx.account}` : ''}{inst.tx.category ? ` · ${inst.tx.category}` : ''}
...
function CheckInBanner({ onReconcile, onQuickAdd, onDismiss }) {
  ...
  return (
    <div style={{ background: 'var(--c-warn-bg)', border: `1px solid var(--c-warn-border)`, borderRadius: 12, padding: '12px 14px', marginBottom: 14 }}>
      <div style={{ fontSize: 12, fontWeight: 700, color: C.orange, marginBottom: 8 }}>{t('checkin.title')}</div>
      <div style={{ fontSize: 11, color: C.textMid, marginBottom: 10 }}>{t('checkin.subtitle')}</div>
      <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
...
function ShareSheet({ book, session, onClose }) {
  ...
  async function sendInvite() {
    ...
    if (!e || !e.includes('@')) { setErr(t('share.invalid_email')); return }
    if (e === session.user.email) { setErr(t('share.own_email')); return }
    ...
    setEmail('')
    setMsg(t('share.invite_sent', { email: e }))
    load()
  }
  ...
  async function removeMember(userId) {
    if (!window.confirm(t('share.confirm_remove'))) return
    ...
    <div style={shS} onClick={onClose}>
      <div style={shI} onClick={e => e.stopPropagation()}>
        <div style={{ padding: '18px 18px 12px', borderBottom: `1px solid ${C.border}`, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <div>
            <div style={{ fontWeight: 700, fontSize: 15 }}>{t('share.title')}</div>
        ...
        <div style={{ padding: '16px 18px', overflowY: 'auto' }}>
          {loading ? <div style={{ color: C.textLow, fontSize: 13 }}>{t('common.loading')}</div> : (
            <>
              {/* Current members */}
              <div style={S.secHead()}>{t('share.members')}</div>
              <div style={{ ...S.card, padding: '4px 14px' }}>
                <div style={{ ...S.row, borderBottom: `1px solid ${C.border}` }}>
                  <div>
                    ...
                    <div style={{ fontSize: 10, color: C.purple, marginTop: 2 }}>{t('share.owner')}</div>
                  </div>
                </div>
                {members.map(m => (
                  <div key={m.id} style={{ ...S.row, borderBottom: 'none' }}>
                    <div>
                      ...
                      <div style={{ fontSize: 10, color: C.textLow, marginTop: 2 }}>{t('share.co_owner')}</div>
                    </div>
                    {isOwner && (
                ...
                {members.length === 0 && (
                  <div style={{ fontSize: 12, color: C.textLow, padding: '10px 0' }}>{t('share.no_coowners')}</div>
                )}
              </div>
              ...
              {invites.length > 0 && (
                <>
                  <div style={S.secHead(C.orange)}>{t('share.pending_invites')}</div>
                  <div style={{ ...S.card, padding: '4px 14px' }}>
                    {invites.map((inv, i) => (
              ...
              {isOwner && (
                <>
                  <div style={S.secHead()}>{t('share.invite_co_owner')}</div>
                  <div style={{ marginBottom: 10 }}>
                    <input style={S.inp} type="email" placeholder={t('share.email_placeholder')}
                      value={email} onChange={e => setEmail(e.target.value)}
...
function NotificationSheet({ session, bookId, onClose }) {
  ...
  async function handleEnable() {
    ...
    setPermission(perm)
    if (perm !== 'granted') { setErr(t('notif.denied')); return }
    ...
    <div style={sheetStyle} onClick={onClose}>
      <div style={sheetInner} onClick={e => e.stopPropagation()}>
        <div style={sheetHeader}>
          <div style={{ fontWeight: 700, fontSize: 15 }}>{t('notif.title')}</div>
          <button onClick={onClose} style={{ background: 'none', border: 'none', color: C.textLow, fontSize: 22, cursor: 'pointer' }}>×</button>
        </div>
        <div style={{ padding: '16px 18px', overflowY: 'auto' }}>
          {!supported ? (
            <div style={{ color: C.textMid, fontSize: 13 }}>{t('notif.not_supported')}</div>
          ) : loading ? (
            ...
            <>
              ...
              <div style={{ ...S.card, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div>
                  <div style={{ fontWeight: 600, fontSize: 14 }}>{subscribed ? t('notif.on') : t('notif.off')}</div>
                  <div style={{ fontSize: 11, color: C.textLow, marginTop: 2 }}>
                    {subscribed ? t('notif.this_device') : t('notif.enable_hint')}
                  </div>
                </div>
                {subscribed
                  ? <button onClick={handleDisable} disabled={saving} style={{ ...S.btn(C.surfaceHigh, true), padding: '8px 14px', fontSize: 12 }}>{t('notif.turn_off')}</button>
                  : <button onClick={handleEnable} disabled={saving || permission === 'denied'} style={{ ...S.btn(), padding: '8px 14px', fontSize: 12, opacity: permission === 'denied' ? 0.5 : 1 }}>
                      {permission === 'denied' ? t('notif.blocked') : saving ? t('notif.enabling') : t('notif.enable')}
                    </button>
                }
              </div>
              {permission === 'denied' && (
              ...
              {/* Settings */}
              <div style={S.secHead()}>{t('notif.alert_settings')}</div>

              <div style={{ ...S.card, display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
                <div>
                  <div style={{ fontSize: 13, fontWeight: 600 }}>{t('notif.bill_reminders')}</div>
                  <div style={{ fontSize: 11, color: C.textLow }}>{t('notif.bill_hint')}</div>
                </div>
              ...
              <div style={{ ...S.card, display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
                <div>
                  <div style={{ fontSize: 13, fontWeight: 600 }}>{t('notif.low_balance')}</div>
                  <div style={{ fontSize: 11, color: C.textLow }}>{t('notif.low_balance_hint')}</div>
                </div>
              ...
              {settings.low_balance_alerts && (
                <div style={{ marginBottom: 14 }}>
                  <div style={S.lbl}>{t('notif.threshold')}</div>
              ...
              <div style={{ marginBottom: 20 }}>
                <div style={S.lbl}>{t('notif.hour')}</div>
  ...
  return (
    <button onClick={onClick} style={{ flex: 1, background: pv.bg, border: `2px solid ${selected ? pv.accent : 'transparent'}`, borderRadius: 12, padding: '10px 8px', cursor: 'pointer', textAlign: 'left', outline: 'none' }}>
      ...
      <div style={{ textAlign: 'center', fontSize: 11, fontWeight: selected ? 700 : 400, color: selected ? pv.accent : pv.textFaint, marginTop: 5 }}>
        {selected && '✓ '}{isDark ? t('theme.dark') : t('theme.light')}
      </div>
    </button>
  )
}
  ...
  return (
    <div style={sheetStyle} onClick={onClose}>
      <div style={{ ...sheetInner, maxHeight: '88vh' }} onClick={e => e.stopPropagation()}>
        <div style={sheetHeader}>
          <div style={{ fontWeight: 700, fontSize: 15 }}>{t('theme.title')}</div>
          <button onClick={onClose} style={{ background: 'none', border: 'none', color: C.textLow, fontSize: 22, cursor: 'pointer' }}>×</button>
        </div>
        <div style={{ padding: '18px 16px 36px', overflowY: 'auto' }}>

          {/* Palette */}
          <div style={{ fontSize: 10, color: C.textLow, letterSpacing: 2, textTransform: 'uppercase', marginBottom: 10 }}>{t('theme.palette')}</div>
          ...
          {/* Mode */}
          <div style={{ fontSize: 10, color: C.textLow, letterSpacing: 2, textTransform: 'uppercase', marginBottom: 10 }}>{t('theme.mode')}</div>
          ...
          {/* Accessibility */}
          <div style={{ fontSize: 10, color: C.textLow, letterSpacing: 2, textTransform: 'uppercase', marginBottom: 10 }}>{t('theme.a11y')}</div>
  ...
  return (
    <div style={sheetStyle} onClick={onClose}>
      <div style={{ ...sheetInner, maxHeight: '80vh' }} onClick={e => e.stopPropagation()}>
        <div style={{ ...sheetHeader }}>
          <div style={{ fontWeight: 700, fontSize: 15 }}>{t('menu.more')}</div>
          <button onClick={onClose} style={{ background: 'none', border: 'none', color: C.textLow, fontSize: 22, cursor: 'pointer' }}>×</button>
        </div>
        <div style={{ padding: '12px 16px', overflowY: 'auto', flex: 1 }}>
          {navItems.map(item => (
            <button key={item.id} onClick={() => { onNavigate(item.id); onClose() }}
              ...
              <div style={{ flex: 1 }}>
                <div style={{ fontWeight: 700, fontSize: 14, color: activeTab === item.id ? C.purple : C.text }}>{t(item.labelKey)}</div>
                <div style={{ fontSize: 11, color: C.textMid, marginTop: 2 }}>{t(item.descKey)}</div>
              </div>
          ...
          {[
            ...
            { labelKey: 'menu.language',      fn: () => { onShowLang(); onClose() } },
          ].map(({ labelKey, fn }) => (
            <button key={labelKey} onClick={fn}
...
function LanguageSheet({ onClose }) {
  const { t, lang, setLang, LANGUAGES: langs } = useT()
  return (
    <div style={sheetStyle} onClick={onClose}>
      <div style={{ ...sheetInner, maxHeight: '80vh' }} onClick={e => e.stopPropagation()}>
        <div style={sheetHeader}>
          <div style={{ fontWeight: 700, fontSize: 15 }}>{t('lang.title')}</div>
          <button onClick={onClose} style={{ background: 'none', border: 'none', color: C.textLow, fontSize: 22, cursor: 'pointer' }}>×</button>
        </div>
        <div style={{ padding: '12px 16px', overflowY: 'auto' }}>
          {langs.map(l => (
            <button key={l.id} onClick={() => { setLang(l.id); onClose() }}
              style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', width: '100%', background: lang === l.id ? C.surfaceHigh : 'none', border: `1px solid ${lang === l.id ? C.purple : C.border}`, borderRadius: 10, padding: '13px 16px', marginBottom: 8, color: lang === l.id ? C.purple : C.text, fontFamily: 'inherit', fontSize: 14, cursor: 'pointer', textAlign: 'left' }}>
              <span style={{ fontWeight: lang === l.id ? 700 : 400 }}>{l.label}</span>
              {lang === l.id && <span style={{ fontSize: 12 }}>✓</span>}
            </button>
          ))}
        </div>
      </div>
...
// ─── FIRST RUN GUIDE ──────────────────────────────────────────────────────────
function getGuideSteps(t) {
  return [
    { id: 'guide-safe-to-spend', title: t('guide.step1.title'), body: t('guide.step1.body') },
    { id: 'guide-tabs',          title: t('guide.step2.title'), body: t('guide.step2.body') },
    { id: 'guide-fab',           title: t('guide.step3.title'), body: t('guide.step3.body') },
    { id: 'guide-menu',          title: t('guide.step4.title'), body: t('guide.step4.body') },
    { id: 'guide-help',          title: t('guide.step5.title'), body: t('guide.step5.body') },
  ]
}
      ...
      >
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
          ...
          <button onClick={next} style={{ background: C.purple, border: 'none', borderRadius: 8, padding: '8px 18px', color: 'var(--c-btn-text)', fontFamily: 'inherit', fontSize: 13, fontWeight: 700, cursor: 'pointer' }}>
            {step === GUIDE_STEPS.length - 1 ? t('guide.done') : t('guide.next')}
          </button>
        </div>
      </div>
    </>
...
function HelpSheet({ onClose, onReplayGuide }) {
  const { t } = useT()
  const HELP_ITEMS = [
    { icon: '⌂', titleKey: 'help.now.title',       bodyKey: 'help.now.body' },
    { icon: '→', titleKey: 'help.ahead.title',      bodyKey: 'help.ahead.body' },
    { icon: '≡', titleKey: 'help.flow.title',       bodyKey: 'help.flow.body' },
    { icon: '⊙', titleKey: 'help.cycles.title',     bodyKey: 'help.cycles.body' },
    { icon: '□', titleKey: 'help.envelopes.title',  bodyKey: 'help.envelopes.body' },
    { icon: '◈', titleKey: 'help.goals.title',      bodyKey: 'help.goals.body' },
    { icon: '↺', titleKey: 'help.reconcile.title',  bodyKey: 'help.reconcile.body' },
    { icon: 'i', titleKey: 'help.projected.title',  bodyKey: 'help.projected.body' },
  ]
  return (
    <div style={sheetStyle} onClick={onClose}>
      <div style={{ ...sheetInner, maxHeight: '88vh' }} onClick={e => e.stopPropagation()}>
        <div style={{ ...sheetHeader }}>
          <div>
            ...
            <div style={{ fontSize: 11, color: C.textLow, marginTop: 2 }}>{t('help.subtitle')}</div>
          </div>
          <button onClick={onClose} style={{ background: 'none', border: 'none', color: C.textLow, fontSize: 22, cursor: 'pointer' }}>×</button>
        </div>
        <div style={{ padding: '12px 18px 32px', overflowY: 'auto', flex: 1 }}>
          ...
          >
            <span style={{ fontSize: 15 }}>↻</span> {t('help.replay')}
          </button>
          {HELP_ITEMS.map(item => (
            <div key={item.titleKey} style={{ display: 'flex', gap: 14, padding: '14px 0', borderBottom: `1px solid ${C.border}` }}>
              <div style={{ fontSize: 20, flexShrink: 0, width: 28, textAlign: 'center', paddingTop: 2, color: C.purple }}>{item.icon}</div>
              <div>
                <div style={{ fontWeight: 700, fontSize: 13, marginBottom: 4 }}>{t(item.titleKey)}</div>
                <div style={{ fontSize: 12, color: C.textMid, lineHeight: 1.65 }}>{t(item.bodyKey)}</div>
              </div>
            </div>
          ))}
        </div>
  ...
  return (
    <div style={sheetStyle} onClick={onClose}>
      <div style={{ ...sheetInner, maxHeight: '88vh' }} onClick={e => e.stopPropagation()}>
        <div style={sheetHeader}>
          <div>
            <div style={{ fontWeight: 700, fontSize: 15 }}>{t('scratch.title')}</div>
            <div style={{ fontSize: 11, color: C.textLow, marginTop: 2 }}>{book.name}</div>
          </div>
          <button onClick={onClose} style={{ background: 'none', border: 'none', color: C.textLow, fontSize: 22, cursor: 'pointer' }}>×</button>
        </div>
        <div style={{ padding: '18px 18px 36px', overflowY: 'auto' }}>
          ...
          <div style={{ ...S.card, marginBottom: 14 }}>
            <div style={{ fontWeight: 700, fontSize: 14, marginBottom: 6 }}>{t('scratch.new_book')}</div>
            <div style={{ fontSize: 12, color: C.textMid, lineHeight: 1.6, marginBottom: 14 }}>
              {t('scratch.new_book_desc')}
            </div>
            <button style={{ ...S.btn(), width: '100%' }} onClick={onNewBook}>
              {t('scratch.open_picker')}
            </button>
          </div>

          {/* Option B: Delete everything in this book */}
          <div style={{ ...S.card, border: `1px solid ${deleteConfirm ? C.red : C.border}` }}>
            <div style={{ fontWeight: 700, fontSize: 14, marginBottom: 6, color: C.red }}>{t('scratch.delete_title')}</div>
            <div style={{ fontSize: 12, color: C.textMid, lineHeight: 1.6, marginBottom: 14 }}>
              {t('scratch.delete_desc', { name: book.name })}
            </div>
            ...
            {success ? (
              ...
              >
                {t('scratch.delete_btn')}
              </button>
            ) : (
              <div style={{ background: C.redBg, border: `1px solid ${C.red}`, borderRadius: 10, padding: '14px' }}>
                <div style={{ fontSize: 13, color: C.red, fontWeight: 600, marginBottom: 4 }}>{t('scratch.confirm_title')}</div>
                <div style={{ fontSize: 11, color: C.textMid, marginBottom: 12 }}>
                  {t('scratch.confirm_desc', { name: book.name })}
                </div>
                ...
                <div style={{ display: 'flex', gap: 8 }}>
                  <button style={{ ...S.btn(C.red), flex: 1 }} onClick={handleReset} disabled={deleting}>
                    {deleting ? t('common.deleting') : t('scratch.yes_delete')}
                  </button>
                  <button style={S.btn(C.surfaceHigh, true)} onClick={() => { setDeleteConfirm(false); setErr(null) }}>{t('common.cancel')}</button>
                </div>
              </div>
            )}
          </div>
...
function FAB({ onTransaction, onBill, onGoal, onNewBook }) {
  ...
  const actions = [
    { labelKey: 'fab.add_tx',    color: C.purple,  fn: onTransaction },
    { labelKey: 'fab.add_bill',  color: C.orange,  fn: onBill },
    { labelKey: 'fab.save_goal', color: C.green,   fn: onGoal },
    { labelKey: 'fab.new_book',  color: C.textMid, fn: onNewBook },
  ]
  return (
    <>
      {open && (
      ...
      <div style={{ position: 'fixed', bottom: 'calc(76px + env(safe-area-inset-bottom))', right: 16, zIndex: 150, display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 8, pointerEvents: 'none' }}>
        {open && (
          <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 8, pointerEvents: 'auto' }}>
            {actions.map(({ labelKey, color, fn }) => (
              <button key={labelKey} onClick={() => { fn(); setOpen(false) }}
      ...
      {showBookPicker && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', zIndex: 200 }} onClick={() => { setShowBookPicker(false); setNewBookName('') }}>
          <div style={{ position: 'absolute', top: 70, left: 16, right: 16, background: C.surface, borderRadius: 12, border: `1px solid ${C.border}`, overflow: 'hidden' }} onClick={e => e.stopPropagation()}>
            <div style={{ fontSize: 10, color: C.textLow, letterSpacing: 2, textTransform: 'uppercase', padding: '12px 14px 6px' }}>{t('books.my_books')}</div>
            {allBooks.map(b => (
            ...
            <div style={{ borderTop: `1px solid ${C.border}`, padding: '10px 14px 14px' }}>
              <div style={{ display: 'flex', gap: 8 }}>
                <input style={{ ...S.inp, flex: 1, fontSize: 13, padding: '8px 10px' }}
                  placeholder={t('books.new_placeholder')} value={newBookName}
      ...
      <div id="guide-tabs" style={{ position: 'fixed', bottom: 0, left: 0, right: 0, background: C.surface, borderTop: `1px solid ${C.border}`, display: 'flex', zIndex: 100, paddingBottom: 'env(safe-area-inset-bottom)' }}>
        {MAIN_TABS.map(tab => (
          <button key={tab.id} onClick={() => setActiveTab(tab.id)}
            ...
            <span style={{ fontSize: 17, lineHeight: 1, color: activeTab === tab.id ? C.purple : C.textMid }}>{tab.icon}</span>
            <span style={{ fontSize: 9, color: activeTab === tab.id ? C.purple : C.textLow, fontWeight: activeTab === tab.id ? 700 : 400, letterSpacing: 0.3 }}>{t(tab.labelKey)}</span>
            {activeTab === tab.id && <div style={{ width: 16, height: 2, background: C.purple, borderRadius: 1 }} />}
          </button>
        ))}
      </div>
      ...
      {showAccountPicker && (
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.7)', zIndex: 800 }} onClick={() => setShowAccountPicker(false)}>
          <div style={{ position: 'fixed', bottom: 0, left: 0, right: 0, background: C.surface, borderRadius: '20px 20px 0 0', padding: '20px 18px 36px' }} onClick={e => e.stopPropagation()}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
              <div style={{ fontSize: 15, fontWeight: 700 }}>{t('reconcile.which')}</div>
  ...
  if (session === undefined || checkingBook) {
    return (
      <div style={{ ...S.root, paddingBottom: 0, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', minHeight: '100vh', gap: 16 }}>
        <Spinner />
        <div style={{ fontSize: 12, color: C.textLow }}>{t('app.loading')}</div>
      </div>
    )
  }
  ...
  if (pendingInvites.length > 0) {
    return (
      <div style={{ ...S.root, paddingBottom: 0, padding: 20 }}>
        <div style={{ fontSize: 18, fontWeight: 700, marginBottom: 4 }}>{t('app.name')}</div>
        <div style={{ fontSize: 12, color: C.textLow, marginBottom: 20 }}>{t('invite.subtitle')}</div>
        {pendingInvites.map(inv => (
          <div key={inv.id} style={{ ...S.card, borderLeft: `3px solid ${C.purple}` }}>
            <div style={{ fontSize: 13, fontWeight: 600, marginBottom: 4 }}>
              {t('invite.invited_to')} <span style={{ color: C.purple }}>{inv.books?.name || t('invite.a_book')}</span>
            </div>
            <div style={{ fontSize: 11, color: C.textLow, marginBottom: 12 }}>
              {t('invite.accept_desc')}
            </div>
            <div style={{ display: 'flex', gap: 8 }}>
              <button onClick={() => acceptInvite(inv)} style={{ ...S.btn(C.purple), flex: 1 }}>{t('invite.accept')}</button>
              <button onClick={() => declineInvite(inv)} style={{ ...S.btn(C.surfaceHigh, true) }}>{t('invite.decline')}</button>
            </div>
          </div>
        ))}
        <button onClick={signOut} style={{ ...S.btn(C.surfaceHigh, true), width: '100%', marginTop: 12 }}>{t('invite.signout')}</button>
      </div>
    )
  }
